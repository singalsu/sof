...
ZTEST(math_complex, test_icomplex32_to_polar)
{
	...
	double magnitude_scale_q30 = 1/1073741824.0;
	double angle_scale_q29 = 1/536870912.0;
	...
	for (i = 0; i < TEST_COMPLEX_POLAR_NUM_POINTS; i++) {
		...
		icomplex32_to_polar(&complex, &polar);
		...
		zassert_true(delta_mag <= MAGNITUDE_ABS_TOL,
			     "Magnitude calc error at (%d, %d)",
	...
	icomplex32_to_polar(&complex_mag_max, &polar);
	printf("delta_mag_max = %g at (%d, %d) -> (%d, %d)\n",
	       delta_mag_max, complex_mag_max.real, complex_mag_max.imag,
	       polar.magnitude, polar.angle);

	icomplex32_to_polar(&complex_ang_max, &polar);
	printf("delta_ang_max = %g at (%d, %d) -> (%d, %d)\n",
	       delta_ang_max, complex_ang_max.real, complex_ang_max.imag,
	       polar.magnitude, polar.angle);
}
...
ZTEST(math_complex, test_ipolar32_to_complex)
{
	...
	double scale_q31 = 1/2147483648.0;
	...
	for (i = 0; i < TEST_COMPLEX_POLAR_NUM_POINTS; i++) {
		...
		ipolar32_to_complex(&polar, &complex);
	...
	/* Re-run worst cases to print info */
	ipolar32_to_complex(&polar_real_max, &complex);
	printf("delta_real_max = %g at (%d, %d) -> (%d, %d)\n", delta_real_max,
	       polar_real_max.magnitude, polar_real_max.angle,
	       complex.real, complex.imag);

	ipolar32_to_complex(&polar_imag_max, &complex);
	printf("delta_iamg_max = %g at (%d, %d) -> (%d, %d)\n", delta_imag_max,
	       polar_imag_max.magnitude, polar_imag_max.angle,
	       complex.real, complex.imag);
}
