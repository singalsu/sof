...
function ref_sofm_fft_multi()
	...
	num_tests = 1;
	scale_q31 = 2^31;
	min_int32 = int32(-scale_q31);
	max_int32 = int32(scale_q31 - 1);
	opt.bits = 32;
	...
	if 1
		input_data_real_q31 = int32(ones(N, num_tests) * scale_q31);
		input_data_imag_q31 = int32(ones(N, num_tests) * scale_q31);
	end
	if 0
		ft = 997;
		fs = 48e3;
		t = (0:(N-1))'/fs;
		x = sin(2*pi*ft*t) .* kaiser(N, 14);
		dither = rand(N, 1) + rand(N, 1) - 1;
		input_data_real_q31 = int32(x * scale_q31 + dither);
