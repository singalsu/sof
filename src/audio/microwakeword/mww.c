 ...
 * speech (vad_flag == 1), chosen so recovery is ~0.5 dB/sec (hop=10 ms, 100 hops/sec).
 */
#define MWW_AGC_GAIN_TARGET_Q23    0 /* 0 dB */
		       ...
		       struct sof_sink **sinks, int num_of_sinks)
{
	...
	while (bytes_to_process >= MWW_HOP_BYTES) {
		...
		hdr = (const struct mfcc_data_header *)data_ptr;
		...
		/* Requantize to int8: skip if no speech in this hop (fill with silence) */
		if (hdr->vad_flag) {
			...
			memset(slice, MEL_CLIP_MIN_Q7, MWW_FEATURE_SIZE * sizeof(int8_t));
		}

		/* Release: creep back toward 0 dB target */
		if (cd->agc_gain_q23 < MWW_AGC_GAIN_TARGET_Q23) {
			cd->agc_gain_q23 += MWW_AGC_RELEASE_STEP_Q23;
			if (cd->agc_gain_q23 > MWW_AGC_GAIN_TARGET_Q23)
				cd->agc_gain_q23 = MWW_AGC_GAIN_TARGET_Q23;
		}
		...
		if (cd->feature_slices_filled >= MWW_FEATURE_SLICE_COUNT) {
			...
			/* VAD gate: skip inference computation if no active speech in any of the 3 slices */
			if (!cd->vad_history) {
				cd->vad_gated_inferences++;
#if CONFIG_COMP_MWW_DEBUG_TRACE
				comp_info(dev, "[MWW DBG] inference skipped: VAD=0 in all slices");
#endif
				continue;
			}
