...
def load_dataset(
    ...
    Applies streaming-aware temporal augmentations:
      ...
      - Random gain augmentation across all speech windows
    """
    rng = np.random.default_rng(seed)
    silence_pool = build_silence_pool(feat_root)

    def sample_silence_window(length: int) -> np.ndarray:
        if silence_pool.shape[0] <= length:
            return np.full((length, HOP_BINS), AGC_CLIP_MIN, dtype=np.float32)
        idx = rng.integers(0, silence_pool.shape[0] - length)
        return silence_pool[idx : idx + length].copy()
    ...
    for label in labels:
        ...
        for f in raw_files:
            ...
            if is_keyword:
                # 1. Positive class: Complete keyword with random start offsets & gain jitter
                for _ in range(jitter_pos_count):
                    win = sample_silence_window(window_hops)
                    gain_lin = 0.0
                    if gain_aug_db_min is not None and gain_aug_db_max is not None:
                        gain_lin = rng.uniform(gain_aug_db_min, gain_aug_db_max) * 0.1
                ...
                if T < window_hops:
                    ...
                    win_p[window_hops - prefix_len :] = mel[:prefix_len]
                    all_X.append(win_p)
                    all_y.append(0)
                    ...
                    win_s[:suffix_len] = mel[T - suffix_len :]
                    all_X.append(win_s)
                    all_y.append(0)
                ...
                # Non-target speech (unknown): temporal jitter & gain augmentation
                for _ in range(jitter_neg_count):
                    win = sample_silence_window(window_hops)
                    gain_lin = 0.0
                    if gain_aug_db_min is not None and gain_aug_db_max is not None:
                        gain_lin = rng.uniform(gain_aug_db_min, gain_aug_db_max) * 0.1
