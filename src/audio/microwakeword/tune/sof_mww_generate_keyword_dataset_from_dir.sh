...
#   GAIN_AUG           1 = apply Gaussian level jitter, 0 = leave baseline (default 1).
#   GAIN_PEAK_DBFS     Peak-normalization target in dBFS (default -10).
#   GAIN_SIGMA_DB      Gaussian jitter sigma in dB (default 5).
#   GAIN_HEADROOM_DB   Headroom below full scale in dB (default 1).
...
PYEOF
	...
	# 4. Gain Jitter
	if [[ "$GAIN_AUG" = "1" ]]; then
		echo "    Applying gain level jitter to $FINAL_DIR (peak=${GAIN_PEAK_DBFS} dBFS, sigma=${GAIN_SIGMA_DB} dB)"
