...
def run_streaming_verification(
    ...
    for label in all_labels:
        ...
        for f in raw_files:
            mel, _ = mww_dataset.load_raw_hops(f)
            if mel.shape[0] < slice_hops:
                continue

            # Apply soft AGC across the whole file
            mel_agc = mww_dataset.apply_soft_agc(mel)

            # Reset interpreter state variables if possible by re-allocating
            interp.allocate_tensors()
