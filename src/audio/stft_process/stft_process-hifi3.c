 ...
 * hot-path helpers.  It is guarded by SOF_USE_HIFI(3, COMP_STFT_PROCESS)
 * so only one of the generic / hifi3 implementations is active.
 */
...
void stft_process_overlap_add_ifft_buffer(struct stft_process_state *state, int ch)
{
	...
	struct stft_process_fft *fft = &state->fft;
	ae_int32x2 *w = (ae_int32x2 *)obuf->w_ptr;
	ae_int32 *fft_p = (ae_int32 *)&fft->fft_buf[fft->fft_fill_start_idx];
	int i, n;
