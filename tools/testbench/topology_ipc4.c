...
int tb_send_switch_control(struct tb_mq_desc *ipc_tx, struct tb_mq_desc *ipc_rx,
			  struct tb_ctl *ctl, int *control_values, int num_values)
{
	struct ipc4_module_large_config config = {{ 0 }};
	...
	/* configure the IPC message */
	tb_ctl_ipc_message(&config, SOF_IPC4_SWITCH_CONTROL_PARAM_ID, data_size,
			   ctl->module_id, ctl->instance_id, SOF_IPC4_MOD_LARGE_CONFIG_SET);
	config.extension.r.final_block = 1;
	config.extension.r.init_block = 1;
